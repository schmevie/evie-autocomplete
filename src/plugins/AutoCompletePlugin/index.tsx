...
import { JSX, SetStateAction, useEffect, useState, Dispatch, useRef, useCallback } from "react";
...
const $textNodeTransform = async function (
    ...
    if (matchString && lastHandledMatchRef.current?.key === node.getKey() && lastHandledMatchRef.current?.match === matchString) {
        return;
    }
...
export const AutoCompletePlugin = function (): JSX.Element | null {
    ...
    const lastHandledMatchRef = useRef<{ key: string; match: string } | null>(null);
    ...
    const handleUpdateSelected = useCallback((value: string) => {
